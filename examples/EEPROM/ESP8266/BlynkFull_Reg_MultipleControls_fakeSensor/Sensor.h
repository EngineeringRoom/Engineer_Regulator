...
/*
  ...
  Будет NAN(не число) или -50, а может вообще при обрыве связи контроллер зависнет
...
// Вот эти функции при работе с реальным сенсором нужно будет реализовать
// сомостоятельно для своих датчиков
...
float readSensor4();        // Непосредственно опрос датчика
...
float ReadSensors(const int &i){
  ...
  switch (i) {
          ...
          case 2:
            ...
            break;
          case 3:
            Temp = readSensor4();        // Читаем температуру от датчика 
            break;
          default:
            Temp = NAN;
            Serial.println(F("There is no such sensor. i is specified incorrectly."));                   
  ...
  if      (Temp >= ALLARM_MAX_TEMP){
     ...
     */
    if(!FlagUpdateValueDisplay[i]){FlagUpdateValueDisplay[i] = true;}
    if(!FlagUpdateProperty[i]){FlagUpdateProperty[i] = true;}
    Error=true;
...
// Вот эти три прототипа функций нужны для работы эмулятора ошибок
...
#define MAX_TEMP4 30 // Максимальная температура
#define MIN_TEMP4 10 // Минимальная температура

// Фейковая Функция опроса датчика
float readSensor4() {
  static float Temp = MIN_TEMP4;
  static bool Direction = true;
