#define MAX_TEMP1 30 // Максимальная температура
...
// Прототипы функций
float loopSensor1();         // Основная функция опроса датчики. Тут же проверки и их решения.
float loopSensorBUG();
...
float loopSensorBUG(){
  ...
  else if (R[0].Alarm()){  // Если предидущие условия несработали и есть сигнал Авария значит            
    R[0].setAuto();        // данные от датчика опять приходят! Значит Возвращаемя в Автоматический режим
  }
